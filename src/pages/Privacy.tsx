...
• **Google LLC (USA) — Google Gemini API (paid tier)**: AI language model used to generate chat responses and reflective insights. Message texts and selected conversation tone are transmitted in real time and processed solely to generate the response. Per Google's Gemini API Additional Terms of Service, inputs and outputs are **not used to train Google's models**. Google acts as a **processor under Art. 28 GDPR** on Soulvay's behalf, governed by the Google Cloud Data Processing Addendum including the EU Standard Contractual Clauses. Additionally covered by the EU-US Data Privacy Framework.
...
• **Google LLC (USA) — Google Gemini API (Paid Tier)**: KI-Sprachmodell zur Generierung von Chat-Antworten und reflektierenden Insights. Übermittelt werden Nachrichtentexte sowie der gewählte Gesprächston in Echtzeit, ausschließlich zur Erzeugung der Antwort. Gemäß Googles Gemini API Additional Terms of Service werden Eingaben und Ausgaben **nicht zum Training von Googles Modellen** verwendet. Google handelt als **Auftragsverarbeiter im Sinne von Art. 28 DSGVO** für Soulvay; Grundlage ist das Google Cloud Data Processing Addendum inklusive der EU-Standardvertragsklauseln. Zusätzlich abgesichert über das EU-US Data Privacy Framework.
